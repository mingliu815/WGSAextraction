...
def extractAnnotation(annotationFile, output, SNP = None, region = None, extract = None, configureFile = None, bgzipOut = False):
	...
	# save header as txt
	cmd_header = "/udd/reliu/bin/htslib/tabix -H {0} > header.txt".format(annotationFile)
...
#                        if "#" in line:
        ...
        if SNP != None:
                snp_split = SNP.split(":")
                cmd_snp = "/udd/reliu/bin/htslib/tabix {0} {1}-{2} -h".format(annotationFile, SNP, snp_split[1])+ ">" + "tabix_result.txt"
		os.system(cmd_snp)
		

	# if given a region chr:posStart-posEnd
	if region != None:
		cmd_region = "/udd/reliu/bin/htslib/tabix {0} {1} -h".format(annotationFile, region)+ ">" + "tabix_result.txt"
        	os.system(cmd_region)

	#if given bed file
	if extract != None:
		cmd_extract = "/udd/reliu/bin/htslib/tabix {0} -R {1} -h".format(annotationFile, extract)+ ">" + "tabix_result.txt"
